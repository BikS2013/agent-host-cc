...
- [ ] A.1 From `<source-repo>/agent-host/`, copy into ``:
  - `src/` (whole tree) → `src/`
  - `test/` (whole tree) → `test/`
  - `package.json` → `package.json`
  - `tsconfig.json` → `tsconfig.json`
  - `vitest.config.ts` → `vitest.config.ts`
  - `Dockerfile` → `Dockerfile`
  - `.dockerignore` (if present in source) → `.dockerignore`
- [ ] A.2 Do **NOT** copy `node_modules/`, `dist/`, `package-lock.json`. The lockfile will be regenerated after rename.
- [ ] A.3 Verify the copy with `find <repo-root> -type f -name '*.ts' | wc -l` and compare against the source count from the codebase scan (sources: 13 src files + 14 test files + 2 fixtures ≈ 29).
- [ ] A.4 Run `grep -RIn "open-webui-phase1" src test package.json Dockerfile` — must produce zero hits. (If the source tree references its own sibling paths, those references should be removed in Phase F.)

### Files created in Phase A
- `src/**` (all TypeScript sources from source `src/`)
- `test/**` (all unit + integration tests + fixtures)
- `package.json`
- `tsconfig.json`
- `vitest.config.ts`
- `Dockerfile`
...
- [ ] B.1 Rename test fixture files:
  - `test/fixtures/mockFoundry.ts` → `test/fixtures/mockAnthropicProvider.ts` and rename the exported function `startMockFoundry` → `startMockAnthropicProvider`.
  - `test/fixtures/mockOpenWebUI.ts` → `test/fixtures/mockFilesApi.ts` and rename the exported function `startMockOpenWebUI` → `startMockFilesApi`.
- [ ] B.2 Update every importer of those fixtures:
  - `test/integration/agentHost.integration.test.ts` (lines ~23, ~33 in source).
...
Renamed:
- `test/fixtures/mockFoundry.ts` → `mockAnthropicProvider.ts`
- `test/fixtures/mockOpenWebUI.ts` → `mockFilesApi.ts`
...
- [ ] D.1 In `package.json`, remove `@fastify/multipart` from `dependencies` (codebase scan §10 confirms it is unused).
- [ ] D.2 In `Dockerfile`, change the runtime stage's user-creation block to:
  ```
...
- [ ] E.1 Update fields in `package.json`:
  - `"name": "agent-host-cc"`
...
- [ ] F.1 Run, from ``:
  ...
  2. **Historical** — relocate the explanation into `docs/reference/historical-context-cc-monitor.md` (created in Phase 5 by the Designer; for now, just leave a `TODO(plan-001-F):` comment if absolutely needed).
...
- [ ] G.1 In `CLAUDE.md`, add a section **"Configuration Fallback Exceptions"** under (or alongside) the `<structure-and-conventions>` block. The section must enumerate the two intentional silent-default fallbacks the design relies on:
...
- [ ] G.3 Verification: `grep -A 12 'Configuration Fallback Exceptions' CLAUDE.md` must show both bullets.
